    ...
    try{
        ...
        foreach($dataRequeste as $index=>$levelkey){
            ...
            if ($toogleCheck=='checked') {
                        ...
                        $check="true";
                        if($ordernumber=="1" && $userStatus=='checked')
                        {
                            foreach ($AllBoothsList as $boothIndex=>$boothValue ){
                                if($boothValue['bootheOwnerID'] == $rolekey ){  
                                    $check="false";
                                }           
                            }
                            if($check=="true")
  ...
  try{
    ...
    $args = array(
        ...
        foreach ($listOFtaskArray as $key => $tasksObject) {
            ...
            if ($value['type'][0] == 'color') {
                $uploadFilesubmit=$_FILES[$value['key'][0]];
                if(!empty($uploadFilesubmit)){

                    $uploadedFileURL = resource_file_upload($uploadFilesubmit);
                    $meta_array[$value['key'][0]]=$uploadedFileURL;
                }
...
function add_contentmanager_js(){
      wp_enqueue_script('safari4', plugins_url().'/EGPL/js/my_task_update.js', array('jquery'),'5.2.7', true);
...
function add_contentmanager_settings() {
    ...
    wp_register_script('adminjs', plugins_url('js/admin-cmanager.js?v=2.39', __FILE__), array('jquery'));
...
function custome_email_send($user_id, $userlogin='', $welcomeemailtemplatename='', $settitng_key=''){
        ...
        foreach($arr as $key=> $a_value) {
            ...
            if($a_value->PrirotyNumber>=$Number && $a_value->Status=='checked')
            {
                ...
                if ($floorPlanSetting["Open_users"] == "Auto and Email") {
                    ...
                    print_r($email_type);
                        custome_email_send($a_value->Id,$email,$email_type,'AR_Contentmanager_Email_Template_booth');
                }
                break;
            }
        }
