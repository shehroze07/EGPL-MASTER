...
jQuery(document).ready(function () {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/orderreport.php?contentManagerRequest=loadorderreport";
  jQuery.ajax({
    url: urlnew,
    cache: false,
    contentType: false,
    ...
    success: function (data) {
      ...
      jQuery.each(columsheader, function (key, value) {
        ...
        if (columsheader[key].data == "Products List") {
          ...
          if (
            columsheader[key].type == "num" ||
            columsheader[key].type == "num-fmt"
          ) {
            columnsheaderarrayfortable.push({
              type: "num",
              title: columsheader[key].title,
      ...
      jQuery.each(columsheader, function (key, value) {
        if (columsheader[key].title != "Order ID") {
          if (columsheader[key].type == "date") {
            jQuery("#builder").queryBuilder("addFilter", {
              id: columsheader[key].title,
              unique: true,
              label: columsheader[key].title,
              type: "date",
      ...
      jQuery.each(columsheader, function (key, value) {
        if (columsheader[key].title == "Order Date") {
          jQuery("#orderbycolumnsname").append(
            '<option value="' +
              columsheader[key].title +
              '"  selected="selected">' +
              columsheader[key].title +
              "</option>"
          );
        ...
        if (
          ...
          columsheader[key].title == "Order Total Amount"
        ) {
          jQuery("#orderreportcolumns").append(
            '<option value="' +
              columsheader[key].title +
              '" selected="selected">' +
              columsheader[key].title +
      ...
      if (data != "") {
          ...
          initComplete: function () {
            this.api()
              ...
              .every(function () {
                ...
                jQuery(".dataTables_filter input").on(
                  "keyup click",
                  function () {
        ...
        if (orderreportstatusloading != undefined) {
          ...
          jQuery("#loadorderreport").val(
            decodeURIComponent(orderreportstatusloading)
          );
...
function updatecurrentordernotes(OrderID) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/orderreport.php?floorplanRequest=getcurrentOrderNote";
  var updatevalue =
    url +
    "wp-content/plugins/EGPL/orderreport.php?floorplanRequest=updatedcurrentordernote";
      ...
      swal(
        {
          title: "Add Note",
          text:
            '<textarea style="font-weight: 400;padding: 10px;width: 80%;min-height: 150px;"class="ordernotetext">' +
            OrderNote.ordernote +
            "</textarea>",
  ...
  if (!jQuery.isEmptyObject(jsondataorder)) {
            ...
            if (oData[key].filteroperator == "between") {
              ...
              jQuery.fn.dataTable.ext.search.push(function (
                ...
                var filterenddatetime = new Date(
                  filterenddate[2] + "-" + enddatemonth + "-" + filterenddate[0]
                ).getTime();
              ...
              jQuery.fn.dataTable.ext.search.push(function (
                ...
                console.log("filterstartdatetime--: " + oData[key].filtervalue);

                if (gettimecol == oData[key].filtervalue) {
                  console.log(gettimecol + "=>" + oData[key].filtervalue);
                  return true;
                }
                return false;
              });
    ...
    for (var key in stringdata) {
      if (stringdata[key].type != "date" && stringdata[key].type != "integer") {
        ...
        querybuild +=
          '.column(":contains(' +
          stringdata[key].filterid +
          ')").search( "' +
          stringdata[key].filtervalue +
          '" )';
      }
    }
    ...
    ordertablereport
      .api()
      .column(":contains(" + orderbycolname + ")")
      .order(orderbytype)
      .draw();
    jQuery.each(jsondataorder.rules, function (key, value) {
      filteroutput +=
    ...
    var tooltiphtml =
      ...
      filteroutput +
      '" style="cursor: pointer;" ><div class="faq-page-cat-icon"><i style="color:#00a8ff !important;" class="reporticon font-icon fa fa fa-filter fa-2x"></i></div><div class="faq-page-cat-title" style="color:#00a8ff"> Filters applied </div><div class="faq-page-cat-txt" id="filteredordercount" >' +
      filterrowscount[0].length +
      "</div></div>";
    ...
    for (var i = 0, iLen = tablesettings[0].aoColumns.length; i < iLen; i++) {
      if (
        ...
        ordertablereport.fnSetColumnVis(
          tablesettings[0].aoColumns[i].idx,
          true
        );
      } else {
        ordertablereport.fnSetColumnVis(
...
function order_report_savefilters() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/orderreport.php?contentManagerRequest=order_report_savefilters";
  var data = new FormData();
    ...
    success: function (data) {
      ...
      jQuery.each(getorderreportsavedlist, function (i, item) {
        if (item == orderreportname) {
          jQuery("#loadorderreportlist").append(
            "<option value='" +
              item +
              "' selected='selected'>" +
              item +
          ...
          jQuery("#customeloadorderreport").append(
            "<option value='" +
              item +
              "' selected='selected'>" +
              item +
...
function confrimremoveorderreport(orderreportname) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/orderreport.php?contentManagerRequest=order_report_removefilter";
  var data = new FormData();
...
function loadorderreport(loadingreportname) {
  ...
  if (dropdownvalue != "defult") {
    ...
    var urlnew =
      url +
      "wp-content/plugins/EGPL/orderreport.php?contentManagerRequest=get_orderreport_detail";
    var data = new FormData();
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        jQuery("#builder").queryBuilder(
          "setRules",
          JSON.parse(getorderreportsavedlist[0])
        );
        jQuery("body").css("cursor", "default");
        jQuery("#orderreportcolumns").empty();
        jQuery("#orderbycolumnsname").empty();
        ...
        if (getorderreportsavedlist[2] == "asc") {
          jQuery("#sortingtype").append(
            '<option value="asc" selected="selected">Asending</option>'
          );
          jQuery("#sortingtype").append(
        ...
        jQuery.each(columsheader, function (key, value) {
          if (
            jQuery.inArray(
              columsheader[key].title,
              JSON.parse(getorderreportsavedlist[1])
            ) !== -1
          ) {
            jQuery("#orderreportcolumns").append(
              '<option value="' +
