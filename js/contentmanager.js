...
function calltoinsertorupdateuser() {
  ...
  var syncurl =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=insertmapdynamicsuser";
  var statustable = "";
  statustable +=
    '<table id="syncuserstatustable" class="display" cellspacing="0" width="100%"><thead><tr><th>Email</th><th>Company Name</th><th>Status</th><th>Result</th><th>Floor plan Exhibitor ID</th></tr></thead><tbody id="syncuserdata">';
  ...
  jQuery("#totaluser").append(
    '<p><i class="fa fa-users" aria-hidden="true"></i>  0/' +
      userids.length +
      "</p>"
  );
    ...
    jQuery.ajax({
      ...
      success: function (data) {
        ...
        if (finalresult.status == "success") {
          ...
          statustable += "<td >" + finalresult.status + "</td>";
          statustable +=
            '<td class="notcreateduser">' + finalresult.result + "</td>";
        ...
        if (countersize.toFixed(0) > 100) {
          ...
          jQuery("#progressreport").append(
            "<p>" + countersize.toFixed(0) + "% done</p>"
          );
        }
...
function embadhelpvidoe() {
  ...
  embedhelplightbox = jQuery.confirm({
    ...
    content:
      ...
      currentsiteurl +
      '/wp-content/plugins/EGPL/js/loading.gif"></p><p id="helpvidep" style="text-align: center;display:none;"><iframe onload="iframeLoaded()" height="600" src="https://help.expo-genie.com/' +
      parseURL[2] +
      '" width="100%"  frameborder="0" allowfullscreen="allowfullscreen"><span data-mce-type="bookmark" style="display: inline-block; width: 0px; overflow: hidden; line-height: 0;" class="mce_SELRES_start">?</span></iframe></p>',
    confirmButton: false,
    cancelButton: false,
    animation: "rotateY",
...
function contact_us() {
  ...
  embedhelplightbox = jQuery.confirm({
    ...
    content:
      '<p id="loadingicon" style="text-align:center;"><img width="50" src="' +
      currentsiteurl +
      '/wp-content/plugins/EGPL/js/loading.gif"></p><p id="helpvidep" style="text-align: center;display:none;"><iframe onload="iframeLoaded()" allowTransparency="true" height="600" height:inherit; overflow:auto;" width="100%" id="contactform123" name="contactform123" marginwidth="0" marginheight="0" frameborder="0" src="https://salesforce.123formbuilder.com/my-contact-form-6050031.html"></iframe></p>',
    confirmButton: false,
    cancelButton: false,
    animation: "rotateY",
    columnClass: "jconfirm-box-container-special",
...
function add_new_sponsor() {
  ...
  if (jQuery("#checknewuser").is(":checked")) {
    var loadwelcomeemailtemplate = jQuery(
      "#selectedwelcomeemailtemp option:selected"
    ).val();
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=add_new_sponsor_metafields";
      ...
      success: function (data) {
        ...
        if (message.msg == "User created") {
          ...
          swal({
            title: "Success",
            text:
              "User added to this site Successfully</br>" +
              message.mapdynamicsstatus,
            type: "success",
            html: true,
            confirmButtonClass: "btn-success",
            confirmButtonText: "Ok",
          ...
          if (
            ...
            ErrorMsg =
              ErrorMsg +
              "<br><p>*Be sure to check for additional spaces before or after the email.</p>";
          }

          jQuery("#sponsor-status").empty();
          jQuery("#sponsor-status").append(
            '<div class="fusion-alert alert error alert-dismissable alert-danger alert-shadow"><span class="alert-icon"><i class="fa fa-lg fa-exclamation-triangle"></i></span>User already exists</div><div class="fusion-clearfix"></div>'
          );
          swal({
            title: "Error",
            text: ErrorMsg,
...
function add_new_admin_user() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=addnewadminuser";
  var data = new FormData();
...
function update_sponsor() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=update_new_sponsor_metafields";
  var data = new FormData();
...
function delete_sponsor_meta(elem, counter) {
  console.log(counter);
  var sName = settingArray.ContentManager["sponsor_name"];
  ...
  if (counter !== 0) {
    swal({
      title: "Are you sure?",
      text: "This user has one or more booths assigned to them. If you wish to remove this user, you must first remove them from their assigned booths.",
      type: "warning",
...
function conform_remove_resource(idsponsor) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=remove_post_resource";
  var data = new FormData();
...
function conform_remove_sponsor(idsponsor) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=remove_sponsor_metas";
  var data = new FormData();
  data.append("id", idsponsor);
  jQuery.ajax({
    url: urlnew,
    ...
    success: function (data) {
      ...
      if (data == "This user removes from this blog successfully") {
        msg =
          "The selected user has been removed from this site. This user may still be present in other sites of your network.";
        title = "Removed";
      } else {
...
function create_new_resource() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=resource_new_post";
  var data = new FormData();
...
function resource_file_upload() {
  ...
  if (file != "") {
    ...
    var urlnew =
      url +
      "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=resource_file_upload";
    jQuery.ajax({
      url: urlnew,
      data: data,
      cache: false,
...
jQuery(document).ready(function () {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=plugin_settings";
  jQuery.ajax({
    url: urlnew,
    data: data,
    cache: false,
...
function update_admin_report() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=update_admin_report";
  var data = new FormData();
    ...
    success: function (data) {
      ...
      jQuery.each(reportData, function (i, item) {
        if (item == reportName) {
          ...
          jQuery("#reportlist").append(
            jQuery("<option/>").attr("value", item).text(item)
          );
        }
      });
...
function old_bulk_import_user() {
  ...
  if (jQuery("#check-1").is(":checked")) {
    var seletwelcomeemailtemplate = jQuery(
      "#selectedwelcomeemailtemp option:selected"
    ).val();
    data.append("welcomeemailstatus", "send");
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=bulkimportuser";
    ...
    success: function (data) {
      ...
      if (message == "faild") {
        ...
        if (message.data == "your sheet is empty.") {
          ...
          console.log(message);
          datatable +=
            '<table id="importuserstatus" class="display" cellspacing="0" width="100%"><thead><tr><th>Email</th><th>Company Name</th><th>Status</th><th>Created User ID</th></tr></thead><tbody id="importuserdata">';
          jQuery.each(message.data, function (index, value) {
            ...
            if (value.created_id != "") {
              ...
              datatable +=
                '<td class="notcreateduser">' + value.status + "</td>";
            }
...
function edit_resource(elem) {
  ...
  jQuery.confirm({
    title: "Edit Resource",
    content:
      '<div id="titlestatus"></div><p>Resource Title   <input style="float: right;border-radius: 3px;padding: 9px;border: #dee8ed solid 1px ; width: 80%; height: 35px;" type="text" id="resourcetitle" value="' +
      resourcetitle +
      '"></p><p>Replace File  <input  type="file" style="width: 80%;float: right;" class="form-control" name="replaceresourcefile" id="replaceresourcefile" required=""></p>',
    ...
    confirm: function () {
      ...
      if (resourcetitle != "") {
        ...
        jQuery("#titlestatus").append(
          '<p style="color:red"><strong>Please fill out the resource title.</strong>'
        );
        return false;
      }
    },
...
function conform_edit_resource(idresource) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=updatresource";
  var data = new FormData();
  jQuery("body").css({ cursor: "wait" });
...
function showprofilefieldupload(e) {
  ...
  if ((reqstatus != "" || reqstatus == "undefined") && reqstatus == "true") {
    var htmlfile =
      ...
      '_fileupload"" id="' +
      name +
      '" name="customefiels[]" required="ture">';
  } else {
    var htmlfile =
      "<input  " +
      specialattributes +
...
function sync_bulk_users() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=GetMapdynamicsApiKeys";
  var syncurl =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=insertmapdynamicsuser";
  var data = new FormData();
  ...
  var useridstml = "";
  useridstml +=
    '<form id="myform" action="' + url + '/sync-to-floorplan/" method="post">';

  for (var i = 0; i < checkedRows.length; i++) {
    jQuery.each(checkedRows[i], function (i, item) {
      if (i == "User ID") {
        useridstml +=
          '<input type="hidden" name="userid[]" value="' + item + '">';
      }
    });
...
function changeuseremailaddress() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=changeuseremailaddress";
  var data = new FormData();
  ...
  jQuery.confirm({
    ...
    content:
      '<div id="titlestatus" ></div><div ><p></p><input placeholder="New Email Address" style="margin-bottom: 10px;padding: 9px;border: #d6e2e8 solid 1px; width: 100%; height: 35px; border-radius: 3px;" type="text" id="newemailaddress" ><p style="color:red;margin: 5px 0px;">This action will also change the login name for this user so we recommend that you send a welcome email message to the new email address.</p><br><p style="margin: 5px 0px;"><input checked type="checkbox" value="checked" id="welcomememailstatus"> Send a welcome email (and new password) to the new email address</p><p><strong>Select Welcome Email Template :</strong><select style="margin-left: 14px;border: #cccccc 1px solid;border-radius: 7px;height: 36px;width: 53%;"id="welcomeemailtemplate">' +
      hiddentemplatelist +
      "</select> </p></div>",
    confirmButtonClass: "mycustomwidth specialbuttoncolor",
    ...
    confirm: function () {
      ...
      if (isValidEmailAddress(newemailaddress)) {
        if (jQuery("#welcomememailstatus").is(":checked")) {
          var selectedtemplateemailname = jQuery(
            "#welcomeemailtemplate option:selected"
          ).val();
        ...
        if (newemailaddress != "") {
                ...
                swal(
                  {
                    title: "Success!",
                    text:
                      "The email and login name for the user has been changed to " +
                      newemailaddress +
                      '. To change any of the other attributes to the user, be sure to make those changes and click "Update".',
                    type: "success",
                    confirmButtonClass: "btn-success",
                  },
                  function (isConfirm) {
          ...
          jQuery("#titlestatus").empty();
          jQuery("#titlestatus").append(
            '<p style="color:red;text-align: center;"><strong>You need to write something!</strong>'
          );
          return false;
        }
      } else {
        jQuery("#titlestatus").empty();
        jQuery("#titlestatus").append(
          '<p style="color:red;text-align: center;"><strong>Error:</strong> This username is invalid because it uses illegal characters. Please enter a valid username.'
        );
        return false;
      }
    },
...
function approvethisuser(elem) {
  ...
  jQuery.confirm({
    ...
    content:
      '<p><h3 style="text-align:center;">Do you want to approve this user? This will send them a welcome email.</h3></p><p style="text-align:center;">Here you can assign a level to this user. IMPORTANT: If you leave as "Unassigned", this user will be prompted to purchase a package before gaining full access to ExpoGenie.</p><p style="margin-bottom: 29px;"><strong>Assign Level :  </strong> <select id="selectassignlevel" style="margin-right: 19px;border: #cccccc 1px solid;border-radius: 7px;height: 36px;width: 53%;float: right;">' +
      jQuery("#assignuserroles").html() +
      '</select></p><p><strong>Select Welcome Email Template :</strong><select style="margin-left: 14px;border: #cccccc 1px solid;border-radius: 7px;height: 36px;width: 53%;"id="welcomeemailtemplate">' +
      hiddentemplatelist +
      '</select> </p><p style="margin: 5px 0px;"><input checked="" type="checkbox" value="checked" id="welcomememailstatus"> Send a welcome email.</p>',
...
function conform_approvethis_user(
  idsponsor,
  userassignrole,
  welcomememailstatus,
  emailtemplatename
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/userreport.php?contentManagerRequest=approve_selfsign_user";
  var data = new FormData();
  ...
  swal(
    {
      ...
      html: true,
      text:
        '<p>You want to decline this user? This will send them a email.</p><br><p>Select Email Template :<select style="margin-left: 14px;border: #cccccc 1px solid;border-radius: 7px;height: 36px;width: 53%;"id="welcomeemailtemplate">' +
        hiddentemplatelist +
        '</select> </p><br><p><input style="width: 12% !important;display: block;height: 20px;margin: 0px;box-shadow: none;" checked="" type="checkbox" value="checked" id="welcomememailstatus"><p style="margin-top: -21px;margin-left: 47px;text-align: left;"> Send a email.</p></p><br>',
      type: "warning",
      showCancelButton: true,
      confirmButtonClass: "btn-danger",
...
function conform_declinethis_user(
  idsponsor,
  emailtemplatename,
  emailsendstatus
) {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/userreport.php?contentManagerRequest=decline_selfsign_user";
  var data = new FormData();
  data.append("id", idsponsor);
  data.append("emailtemplatename", emailtemplatename);
...
function checkemailaddressalreadyexist() {
  ...
  if (currentemail != "") {
    ...
    var urlnew =
      url +
      "wp-content/plugins/EGPL/userreport.php?contentManagerRequest=checkuseralreadyexist";
    var updateurl =
      url +
      "wp-content/plugins/EGPL/userreport.php?contentManagerRequest=updateuserforthissite";
    var data = new FormData();
      ...
      success: function (data) {
        ...
        if (data.trim() == "This email address doesnt exist") {
          ...
          jQuery.confirm({
            title: "Email Status",
            content:
              '<div id="titlestatus" ></div><div ><p></p><input value="' +
              currentemail +
...
function getimportmapping_data() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=bulkimportmappingcreaterequest";
  jQuery.ajax({
    url: urlnew,
    data: data,
    cache: false,
...
function portalsettings_update() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=portalsettingsupdate";
  jQuery.ajax({
    url: urlnew,
    data: data,
    cache: false,
...
function setlevelspriorities() {
  ...
  var urlnew =
    url +
    "wp-content/plugins/EGPL/egpl.php?contentManagerRequest=setlevelspriorities";
  jQuery.ajax({
    url: urlnew,
    data: data,
    cache: false,
